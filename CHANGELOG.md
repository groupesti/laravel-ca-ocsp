...
- Integrated `CA\Log` structured logging via `CaLog` facade across all OCSP services.
- OCSP status resolutions (`good`, `revoked`, `unknown`) are now logged with `CaLog::ocspResponse()`.
- Errors and exceptions in OCSP request parsing, response generation, key decryption, and response storage are logged with `CaLog::critical()`.
