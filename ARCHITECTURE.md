...
**CertificateStatusResolver** -- Takes a parsed CertID (issuer name hash, issuer key hash, serial number) and resolves the certificate's current status (`good`, `revoked`, `unknown`) by querying the certificate database. Returns a `CertStatusResult` DTO with revocation details if applicable. Logs every resolution outcome via `CaLog::ocspResponse()` and catches/logs exceptions via `CaLog::critical()` before re-throwing.
...
- **Structured logging via CA\Log** (2026-03-29): All OCSP operations are logged through the `CaLog` facade (`groupesti/laravel-ca-log`) rather than raw Laravel `Log` calls. This provides structured, auditable logs with consistent context (serial numbers, CA IDs, operation types) and integrates with the broader CA ecosystem logging pipeline.
